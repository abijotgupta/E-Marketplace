...
public class DemandDaoImpl implements DemandDao{
	...
	public boolean addProduct(String userId, String prodId, int demandQty) {
		...
		try {
			ps = con.prepareStatement("select * from user_demand where userId=? and productId=?");
	...
	public boolean removeProduct(String userId, String prodId) {
		...
		try {
			...
			if(rs.next()) {
				...
				//System.out.println("userId "+userId+"\nprodId: "+prodId);
				ps2 = con.prepareStatement("delete from  user_demand where userId=? and productId=?");
	...
	public List<DemandBean> haveDemanded(String prodId) {
		...
		try {
			ps = con.prepareStatement("select * from user_demand where productId=?");
			ps.setString(1, prodId);
			rs = ps.executeQuery();
			
			while(rs.next()) {
				
				DemandBean demand = new DemandBean(rs.getString("userId"),rs.getString("productId"),rs.getInt("quantity"));	
