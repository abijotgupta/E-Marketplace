...
import com.utility.MyCon;



public class Dao {

		int insert(String name,long number,String dob,String email,String pass) 
		{
			try{

				Connection con=MyCon.dbcon();
			
				PreparedStatement st=con.prepareStatement("INSERT INTO accounts(name, number, dob, email, pass) VALUES(?,?,?,?,?)");
				st.setString(1,name);
				st.setLong(2,number);
				st.setString(3, dob);
