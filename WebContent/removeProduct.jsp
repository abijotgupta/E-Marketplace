	...
	<div class="products" style="background-color: #E6F9E6;">  
		<div class="row text-center" >
			...
			for(ProductBean product : products) {
				String remove = null;
				if(isValidUser) {
					remove = "removeProduct.jsp";
				}
				else {
					remove = "adminLogin.jsp";
				}	
			%>
			<div class="col-sm-4">
				<div class="thumbnail">
					<img src="./img?id=<%=product.getProdId() %>" alt="Product" style="height:200px; max-width:200px; max-width:300px;">
					<p class="productname"><%=product.getProdTitle() %>  </p>    <!-- ( <%=product.getProdId() %> ) --> 
					<p class="productinfo"><%=product.getProdSlug() %></p>
					<p class="priceProduct">Rs <%=product.getProdPrice() %> </p>
					<form method="post"> 
						<button type="submit" method="post" formaction="./removeProduct?prodid=<%=product.getProdId() %>">Remove product</button>
					</form> 
				</div>
			</div>
			<% } %>
